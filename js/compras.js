		...
		z = function (n) {
			...
			var q = {
				...
				b = function (a) {
					...
					if (i(a, "object")) return b.extend(k, a)
				},
				x, y;
			b.extend = function (a, d) {
				var c;
				e(d) && (d = a, a = b);
				for (c in d) Object.prototype.hasOwnProperty.call(d, c) &&
					(a[c] = d[c]);
				return a
			};
			b.extend({
			...
			b.extend({
				...
				add: function (a, d) {
					var c = new b.Item(a || {}),
						g = !0,
						r = !0 === d ? d : !1;
					if (!r && (g = b.trigger("beforeAdd", [c]), !1 === g)) return !1;
				...
				find: function (a) {
					var d = [];
					if (i(o[a], "object")) return o[a];
					if (i(a, "object")) return b.each(function (c) {
						var g = !0;
			...
			b.extend({
				writeCart: function (a) {
					...
					t.append(d);
					c = 0;
					for (h = k.cartColumns.length; c < h; c += 1) f = B(k.cartColumns[c]), a = "item-" +
				...
				createCartRow: function (a, d, c, g, r) {
					...
					r.append(d);
					r = 0;
					for (c = k.cartColumns.length; r < c; r += 1) e = B(k.cartColumns[r]), f = "item-" + (e.attr || (i(e.view, p) ? e.view : e.label || e.text || "cell")) + " " + e.className, j = a, j = (h(e.view) ? e.view : i(e.view,
			...
			b.Item = function (a) {
				...
				l += 1;
				for (c.id =
					c.id || "SCI-" + l; !e(o[c.id]);) l += 1, c.id = "SCI-" + l;
				g.get = function (a, b) {
					var d = !b;
			...
			b.Item._ = b.Item.prototype = {
				...
				remove: function (a) {
					if (!1 === b.trigger("beforeRemove", [o[this.id()]])) return !1;
					delete o[this.id()];
					a || b.update();
					return null
				},
			...
			b.extend({
				checkout: function () {
					if ("custom" === k.checkout.type.toLowerCase() && h(k.checkout.fn)) k.checkout.fn.call(b, k.checkout);
			...
			n = {
				...
				trigger: function (a, b) {
					...
					if (!e(this._events[a]) && h(this._events[a][0])) {
						g = 0;
						for (f = this._events[a].length; g < f; g += 1) c = this._events[a][g].apply(this, b || [])
					}
					return !1 === c ? !1 : !0
				}
			};
			...
			b.extend({
				currency: function (a) {
					if (i(a, p) && !e(A[a])) k.currency = a;
					else if (i(a, "object")) A[a.code] = a, k.currency = a.code;
					else return A[k.currency]
				}
			});
			b.extend({
							...
							e.attr("class") && (e.attr("class").match(/item_.+/) && !e.attr("class").match(/item_add/)) && b.each(e.attr("class").split(" "), function (b) {
								...
								if (b.match(/item_.+/)) {
									...
									switch (e.tag().toLowerCase()) {
										...
										case "select":
											d = e.attr("type");
											if (!d || ("checkbox" === d.toLowerCase() || "radio" === d.toLowerCase()) && e.attr("checked") || "text" === d.toLowerCase()) c = e.val();
											break;
										case "img":
											c = e.attr("src");
											break;
			...
			(function () {
				if (f.readyState === "complete") return setTimeout(b.init, 1);
...
(function () {
		...
		switch (typeof j) {
			...
			case "object":
				...
				if ("[object Array]" === Object.prototype.toString.apply(j)) {
					m = j.length;
					for (q = 0; q < m; q += 1) s[q] = p(q, j) || "null";
					o = 0 === s.length ? "[]" : h ? "[\n" + h + s.join(",\n" + h) + "\n" + w + "]" : "[" + s.join(",") + "]";
					h = w;
					return o
				}
				if (n && "object" === typeof n) {
					m = n.length;
					for (q = 0; q < m; q += 1) "string" === typeof n[q] && (l = n[q], (o = p(l, j)) && s.push(f(l) + (h ? ": " : ":") + o))
				} else
					for (l in j) Object.prototype.hasOwnProperty.call(j, l) && (o = p(l, j)) && s.push(f(l) + (h ? ": " : ":") + o);
				o = 0 === s.length ? "{}" : h ? "{\n" + h + s.join(",\n" +
					h) + "\n" + w + "}" : "{" + s.join(",") + "}";
				h = w;
				return o
	...
	"function" !== typeof JSON.stringify && (JSON.stringify = function (e, f, i) {
		...
		v = h = "";
		if (typeof i === "number")
			for (l = 0; l < i; l = l + 1) v = v + " ";
		else typeof i === "string" && (v = i);
		if ((n = f) && typeof f !== "function" && (typeof f !== "object" || typeof f.length !== "number")) throw Error("JSON.stringify");
		return p("", {
			"": e
		})
	});
	"function" !== typeof JSON.parse && (JSON.parse = function (e, f) {
		function h(e, i) {
			...
			if (j && typeof j === "object")
				for (l in j) Object.prototype.hasOwnProperty.call(j, l) && (m = h(j, l), m !== void 0 ? j[l] = m : delete j[l]);
			return f.call(e, i, j)
		}
		var l, e = "" + e;
		i.lastIndex = 0;
		...
		if (/^[\],:{}\s]*$/.test(e.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g, "@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
				"]").replace(/(?:^|:|,)(?:\s*\[)+/g, ""))) return l = eval("(" + e + ")"), typeof f === "function" ? h({
			"": l
		}, "") : l;
